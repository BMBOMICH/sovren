...
```
./sovren examples/hello.sov -o /tmp/hello
/tmp/hello
```

The files in `examples/` are yours to change: `hello.sov` to start,
`play.sov` to poke at, `show.sov` for every word in one place, `mysite.sov`
for a page.
...
A `return` in `main` is an exit code, on every machine:
...
prints `done` and the program ends with code 7.

---

## Leave

`leave` ends the nearest loop around it. `stop` ends the whole
program, `leave` ends only the loop.
...
That prints `5`. Inside nested loops `leave` steps out of the
innermost one only. Leaving across a `try` or `except` is an
error, because the error frame would be left behind.
...
| `string` | join, cut, find, compare. `str_at` gives the byte at a spot, as a number, not a one-letter text |
...
| `lz` / `lz2` | squeeze text down: `lz_pack`, `lz2_pack`, and `lz2h_pack`, the one with huffman codes |
| `run` | run a program and read what it printed: `run_sh`, `run_out`, `compile_sov` |
| `arm` | a whole arm64 machine: `arm_new`, `arm_load`, `arm_run` |
| `browser` | read a page from a site: `web_get`, `web_read` |
...
It runs programs from other toolchains too. A statically linked binary from
a mainstream toolchain runs and prints exactly what it prints natively,
including its allocator, sorts with callbacks, formatted printing, and the
string functions, which such toolchains write with vector instructions.
...
```
./sovren tests/windows-run.sov -o run     then    ./run app.exe
./sovren tests/macos-run.sov -o run         then    ./run app
```

Both are strict on purpose. A Windows program that reaches a raw `syscall` is stopped,
because real Windows has no Linux kernel behind it — a compatibility layer lets that through and the
...
It also covers the stack-based maths unit, the one a compiler reaches for when
...
`tests/x86ins.sov` keeps that check in the suite.
...
back to a helper when one is on the machine, and `web_why` says what
...
walk paths through arrays and objects, and `json_str`/`json_num` keep the
flat meaning.
...
`use proc` starts other programs and reads the clock.

```
proc_run("/bin/echo")        run to the end, give back the exit code
proc_run_us(path)            the same, but give back the child's CPU
                             microseconds (from wait4's rusage - a noisy
                             wall clock cannot bend it)
proc_run_out(path)           the same, but give back everything the
                             child printed as one text (up to 65520 bytes)
proc_fork / proc_wait_pid    the pieces, if you want them by hand
```

`use time` reads the clock: `time_ms()` is thousandths of a second,
`time_now_ns()` the full count, `sleep_ms(n)` pauses.

---

## The check battery

`tests/run.sov` is the whole test battery, written in Sovren. It
...
reports ok, FAIL or skip for each check. The only non-Sovren parts
left are external tools some checks drive as reference opponents and hosts.

```
./sovren tests/run.sov -o /tmp/sov-run && /tmp/sov-run
```

`tests/all.sov` is the fuller battery: everything above plus the
editor, the browser, the os scaffold and the packers. 307 checks,
all green.
...
`use lz2` squeezes text to about half and back, exactly. `lz` looks
back 1024 bytes; `lz2` looks back 4096, waits one byte when waiting
pays, and `lz2h_pack` adds huffman codes over the result, near
deflate class. `lz2r_pack` range-codes the same stream with a table
that learns while it runs, a little tighter still — dist ships it.
A length that lies is refused, not believed.

```
use lz2

main:
    d: file_read_all("book.txt")
    p: lz2h_pack(d, str_len(d))
    print lz2h_plen(p)                the packed size
    print str_eq(lz2h_unpack(p), d)   1, the same bytes back

    q: lz2r_pack(d, str_len(d))
    print lz2r_plen(q)                usually a little smaller
    print str_eq(lz2r_unpack(q), d)   1 again
```

A program can ship packed too. In the os below, `pack hello.bin`
writes `hello.bin.szv` at a tenth the size, and `run hello.bin.szv`
loads it the same as the plain one: `arm_load` unpacks anything
that does not begin with the ELF magic, zeros and all.

`dist.sov` packs the whole repository as one file and proves every
byte unpacks identical:

```
./sovren dist.sov -o dist && ./dist pack    write /tmp/sov-dist.szv
./dist check                                330 files, identical
```
...
`edit.sov` is a small editor with a run key. ctrl-s saves, ctrl-r
saves, compiles the file with the real compiler, runs it and shows
what it printed, ctrl-q quits.

```
./sovren edit.sov -o edit && ./edit mine.sov
```
...
`os.sov` is a small shell over the arm machine: programs are real
android-built binaries, loaded and run by the emulator, and the
`pack` command ships them compressed.

```
./sovren os.sov -o os-check && ./os-check
run hello.bin
pack hello.bin
run hello.bin.szv
exit
...
**Globals do not stay put across deep calls.** Handshake state
lives in a buffer owned by `tl_client` and passed by argument, never
read back from globals between jobs; the
only globals left are written immediately before a read.

**A local may not share a global's name.** Assigning to a global
from inside a job is legal and writes the global. What must never
happen is a *local* appearing under a global's name mid-job: the
load and store ladders divert a global-named name to the global
until a local exists, so the one place a local could materialise
late was the `except` name, silently splitting every earlier and
later use of that name in the job. Since pass 68 the compiler
refuses the program (`Error: the except name shadows a global`)
instead of miscompiling it. Parameters are registered before the
job's first line, so a parameter that shares a global's name
shadows it consistently for the whole job — the compiler's own
emitter helpers (`bs`, `oc`) rely on exactly that.

**The x25519 ladder is shaped to the compiler's strengths.** In
`library/x25519.sov` every job is small with at
